    ...
    statusLabelLoadImage: (state) => {
        ...
        if (state.error) {
            if (state.error.code === 'IMAGE_TOO_SMALL') {
                return 'Le dimensioni minime del media sono {minWidth} &times; {minHeight}';
        ...
        if (state.task === 'blob-to-bitmap') return 'Preparazione del media in corso&hellip;';
        return 'Caricamento del media in corso&hellip;';
    },

    statusLabelProcessImage: (state) => {
        if (!state || !state.task) return undefined;
        if (state.task === 'store') {
            if (state.error) return "Errore durante l'upload del media";
            return 'Upload del media in corso&hellip;';
        }
        if (state.error) return "Errore durante l'elaborazione del media";
        return 'Elaborazione del media in corso&hellip;';
    },
