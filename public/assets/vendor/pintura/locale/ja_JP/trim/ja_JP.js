export default {
    ...
    statusLabelLoadImage: (state) => {
        if (!state || !state.task) return 'メディア待ち';
        if (state.error) {
            if (state.error.code === 'IMAGE_TOO_SMALL') {
                return '最小メディアサイズは {minWidth} × {minHeight} です';
            } else if (state.error.code === 'VIDEO_TOO_SHORT') {
                const unit = state.error.metadata.minDuration === 1 ? '秒' : '秒';
                return `最小ビデオの長さは {minDuration} ${unit} です`;
            } else {
                return 'メディアの読み込みエラー';
            }
        }
        if (state.task === 'blob-to-bitmap') return 'メディアを準備中&hellip;';
        return 'メディアを読み込み中&hellip;';
    },

    statusLabelProcessImage: (state) => {
        if (!state || !state.task) return undefined;
        if (state.task === 'store') {
            if (state.error) return 'メディアのアップロードエラー';
            return 'メディアをアップロード中&hellip;';
        }
        if (state.error) return 'メディアの処理エラー';
        return 'メディアを処理中&hellip;';
    },

    cropLabelCropBoundaryEdge: 'メディアの端',
};
