export default {
    ...
    trimLabelMute: 'Ljud av',
    trimLabelUnmute: 'Ljud på',
    ...
    statusLabelLoadImage: (state) => {
        if (!state || !state.task) return 'Väntar på media';
        if (state.error) {
            if (state.error.code === 'IMAGE_TOO_SMALL') {
                return 'Minimum media storlek är {minWidth} &times; {minHeight}';
            } else if (state.error.code === 'VIDEO_TOO_SHORT') {
                const unit = state.error.metadata.minDuration === 1 ? 'sekund' : 'sekunder';
                return `Minsta videolängd är {minDuration} ${unit}`;
            } else {
                return 'Fel vid laddning av media';
            }
        }
        if (state.task === 'blob-to-bitmap') return 'Förbereder media&hellip;';
        return 'Laddar media&hellip;';
    },

    statusLabelProcessImage: (state) => {
        if (!state || !state.task) return undefined;
        if (state.task === 'store') {
            if (state.error) return 'Fel vid uppladdning av media';
            return 'Laddar upp media&hellip;';
        }
        if (state.error) return 'Fel vid bearbetning av media';
        return 'Bearbetar media&hellip;';
    },

    cropLabelCropBoundaryEdge: 'Media-kant',
};
